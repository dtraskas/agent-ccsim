...
/**
 * HandlerPlugin_Exp2.java
 *
 * Created on 27/07/2008, 14:44
 * 
 * Dimitrios Traskas
...
public class HandlerPlugin_Exp2 extends ComponentPlugin
{
    ...
    private Vector handlerAddresses;
    ...
    protected void setupSubscriptions() 
    {
        ...
        // Subscribe to the Agent forward messages
    ...
    private void processCall(CallInfo call)
    {
        ...
        if (canHandleSkill(skill) && isOnShift()) {
            handleCall(call);                         
    ...
    private void forwardCall(CallInfo call)
    {
        MessageAddress handler = MessageAddress.getMessageAddress(getHandlerId());
        sendMsg(handler, new InfoPacket("FORWARD_CALL", call), MessageType.CallForward);  
    }
    ...
    private String getHandlerId()
    {
        ...
        String handlerId = (String)handlerAddresses.get(index);
        return handlerId;
    }
    ...
    private void getData()
    {
        for (Iterator iter = simDataReceiver.getChangedCollection().iterator(); iter.hasNext();) {
            ...
            this.handlerAddresses = new Vector();
            for(int i=1; i<=totalHandlers;i++){
                String handlerId = "HA_" + i;
                if (handlerId.compareTo(agentId.getAddress()) != 0){
                    handlerAddresses.add(handlerId);
                }
            }
            resetAvailability();
    ...
    private void receiveResetCall()
    {
        for (Iterator iter = simResetReceiver.getAddedCollection().iterator(); iter.hasNext();) {
            ...
            if (relay != null) resetAvailability();
        }
    }
